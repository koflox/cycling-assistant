...
import com.koflox.session.completion.R
...
fun SessionCompletionRoute(
    ...
    val viewModel: SessionCompletionViewModel = hiltViewModel()
...
private fun SessionCompletionTopBar(
    ...
    val title = when (uiState) {
        is SessionCompletionUiState.Content -> uiState.destinationName ?: stringResource(R.string.completion_free_roam_title)
        ...
        actions = {
            if (uiState is SessionCompletionUiState.Content) {
                IconButton(onClick = onShareClick) {
                    Icon(imageVector = Icons.Default.Share, contentDescription = stringResource(R.string.completion_share_action))
                }
            }
        },
...
private fun SessionCompletionLayout(
    ...
    Column(modifier = modifier) {
        Box(
            ...
            MapHeaderOverlay(
                title = uiState.destinationName ?: stringResource(R.string.completion_free_roam_title),
        ...
        SessionSummaryCard(
            ...
            title = stringResource(StatsR.string.stats_config_collected_data_title),
            onEditClick = { onNavigateToStatsConfig(StatsDisplaySection.COMPLETED_SESSION.name) },
